...
export class ProductsService {
  ...
  async listProducts(
    ...
    let countQuery = this.supabase.from("produkty").select("*", { count: "exact", head: true }).eq("user_id", userId);
    ...
    let dataQuery = this.supabase.from("produkty").select("*").eq("user_id", userId);
   ...
   * @private
   */
  private calculatePagination(page: number, limit: number, total: number): PaginationMetaDTO {
   ...
   */
  async getProductById(userId: string, productId: string): Promise<ProductDTO | null> {
  ...
  async deleteProduct(productId: string): Promise<void> {
    ...
    const { error: deleteError } = await this.supabase.from("produkty").delete().eq("id", productId);
