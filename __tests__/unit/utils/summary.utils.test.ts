...
describe('summary.utils', () => {
  describe('recalculateSummary', () => {
    ...
    it('sortuje kategorie malejąco według total_expense', () => {
      ...
      expect(result.by_category[0].total_expense).toBe(15.00); // cat-2 (największy)
      expect(result.by_category[1].total_expense).toBe(10.00); // cat-3
      expect(result.by_category[2].total_expense).toBe(5.00);  // cat-1 (najmniejszy)
    });
    ...
    it('obsługuje ceny z wieloma miejscami po przecinku', () => {
      ...
      expect(result.total).toBe(2.00); // Zaokrąglone do 2.00
    });
