  ...
  describe("receiptFileSchema", () => {
    ...
    describe("niepoprawny rozmiar pliku", () => {
      it("odrzuca plik większy niż 10MB", () => {
        const file = createMockFile("receipt.jpg", 10 * 1024 * 1024 + 1, "image/jpeg");
        const result = receiptFileSchema.safeParse({ receipt: file });
      ...
      it("odrzuca plik o rozmiarze 15MB", () => {
        const file = createMockFile("receipt.jpg", 15 * 1024 * 1024, "image/jpeg");
        const result = receiptFileSchema.safeParse({ receipt: file });
      ...
      it("odrzuca plik o rozmiarze 20MB", () => {
        const file = createMockFile("receipt.jpg", 20 * 1024 * 1024, "image/jpeg");
        const result = receiptFileSchema.safeParse({ receipt: file });
    ...
    describe("edge cases", () => {
      it("odrzuca plik JPEG większy niż 10MB mimo poprawnego typu", () => {
        const file = createMockFile("large-receipt.jpg", 11 * 1024 * 1024, "image/jpeg");
        const result = receiptFileSchema.safeParse({ receipt: file });
